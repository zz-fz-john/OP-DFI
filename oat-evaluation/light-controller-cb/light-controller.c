...
void __attribute__((annotate("critical function"))) listen_to_events(const char *data, int controllerId, int callbackId, void *context __attribute__((unused)))
{
    struct timespec now_ts;
    clock_gettime(CLOCK_MONOTONIC, &now_ts);
    uint64_t now = now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000;
    bool __attribute__((annotate("nova_sensitive_var"))) memory_added_pattern = false;

    printf(" %s \n", __func__);

    for (size_t device_id = 0; device_id < DEVICE_COUNT; device_id++) {
        const RoutedDevice* __attribute__((annotate("nova_sensitive_var"))) device_routing = &DEVICE_ROUTINGS[device_id];
        ...
        if (strncmp(data, device_routing->switchPrefix, prefix_length) == 0) {
            ...
            MethodReact __attribute__((annotate("nova_sensitive_var"))) method = REACT_TURNON;
        ...
        if (strncmp(data, device_routing->switchPrefix, prefix_length) == 0) {
            const char* __attribute__((annotate("nova_sensitive_var"))) method_start = data + prefix_length;
